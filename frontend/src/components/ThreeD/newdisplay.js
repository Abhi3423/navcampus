...
export default function NewDisplay() {
  ...
  const {
    setDisplay, startmodelData, endmodelData
  } = useMapStore();
  ...
  const [directionalIntensity1, setDirectionalIntensity1] = useState(2.0);
  const [directionalColor1, setDirectionalColor1] = useState("#ffffff");
  const [directionalPosition1, setDirectionalPosition1] = useState({ x: 100, y: 200, z: -300 });
  const [directionalIntensity2, setDirectionalIntensity2] = useState(2.0);
  const [directionalColor2, setDirectionalColor2] = useState("#ffffff");
  const [directionalPosition2, setDirectionalPosition2] = useState({ x: -100, y: 200, z: 300 });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (controlsRef.current) {
        controlsRef.current.object.position.set(-18.07618451822854, 156.47738746168923, 500.0303654092317);
        clearInterval(interval); // Stop checking once it's set
      }
    }, 100); // Check every 100ms until controlsRef is available
      ...
      <div className="relative w-full h-full">
        ...
        >
          ...
          <group scale={[scale, scale, scale]}>
            {/* Start Floor at Ground Level */}
            <Model modelData={startmodelData} scale={1} position={[0, 0, 0]} />

            {/* End Floor Positioned Below the Start Floor */}
            <Model modelData={endmodelData} scale={1} position={[0, -floorHeight, 0]} />
          </group>
